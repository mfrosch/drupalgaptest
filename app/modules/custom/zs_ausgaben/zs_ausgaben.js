...
//        title_arguments: [1],
	...
	{
		title: 'Bergsteiger',
		page_callback: 'zs_ausgaben_ausgabe_voll_page',
		page_arguments: [1],
	};
	return items;
}
...
{
	// require more - first as new - further for slider 
	var args = {
		limit: 33 // 32 to show 1 for new
	};
	...
	zs_ausgaben_get_issues(
	{
	    ...
	    success: function(issues) 
	    {
	    	issues.forEach(function(issue) {
	    		...
	    		{
		    		first = false;
			    	var imgpath = 'http://bergsteiger.de/sites/bergsteiger.de/files/bilder/cover/600/' + artikelnr + '.jpg';
	    		...
	    		{
		    		var issuehtml = __getIssuePreviewHtml(issue);
	...
	var currentissue = '<div class="issue neu" style="display: none">' 
			...
			+ '<div class="ausgabe">Aktuelle Ausgabe</div>'
		+ '</div>';
	
	currentissue = currentissue + '<div class="subhead">'
	+ '<span class="label">Alle Ausgaben</span><span class="showall">' + l('Alle anzeigen', 'alle') + '</span>'
	+ '</div><div class="issues_container slickme">'
	+ '</div>';
	
	return currentissue;
}

function __getIssuePreviewHtml(issue)
{
	var title = issue.web_headline;
	var artikelnr = issue.artikelnr;
	var auflageid = issue.auflage_id;
	...
	return '<div class="issue">' 
			...
			+ '<div class="ausgabe">Ausgabe ' + issue.auflagename + '</div>'
		+ '</div>';		
}
...
*/
function zs_ausgaben_ausgabe_page(auflage) 
{
	try {
	    if (auflage) 
	    {	
	    	...
	    	zs_ausgaben_get_issue({
			    ...
			    success: function(issue) {
			    	...
			    	var path_full = 'ausgabe/' + artikelnr + '/voll';
			    	var path_test = 'ausgabe/' + artikelnr + '/leseprobe';
			    	
			    	$('.issue.single.ausgabe-' + auflage + ' .issue_img').attr('src', imgpath);
			    	$('.issue.single.ausgabe-' + auflage + ' .issue_img').attr('alt', title);
			    	$('.issue.single.ausgabe-' + auflage + ' .issue_img').attr('title', title);
			    	
			    	$('.issue.single.ausgabe-' + auflage + ' .btn-voll').attr('onclick', $('.issue.single.ausgabe-' + auflage + ' .btn-voll').attr('onclick').replace('_replaceme_', path_full));
			    	$('.issue.single.ausgabe-' + auflage + ' .btn-probe').attr('onclick', $('.issue.single.ausgabe-' + auflage + ' .btn-probe').attr('onclick').replace('_replaceme_', path_test));

			    	$('.issue.single.ausgabe-' + auflage + ' .issue_head').text(title);
			    	$('.issue.single.ausgabe-' + auflage + ' .issue_date').text(issuedate);
			    	$('.issue.single.ausgabe-' + auflage).fadeIn();
			    }
			});	   
	    	...
	    	var issue = '<div class="issue single ausgabe-' + auflage + '" style="display: none">' 
...
//			    		+ '<a id="full2" data-role="button" onclick="window.open(\'http://www.crusoemedia.com/kunden/test.pdf\', \'_system\', \'location=yes\')" data-theme="b">PDF system</a>'
			    		+ bl('Ausgabe kaufen', '_replaceme_', { attributes: { 'class': 'btn-voll' } })
			    		+ bl('Leseprobe öffnen', '_replaceme_', { attributes: { 'class': 'btn-probe' } })
//			    		+ bl('Leseprobe öffnen extern', 'http://zs.vh-digital.de/sites/selbermachen.de/files/blick-ins-heft/911509/index.html#undefined', { InAppBrowser: true })
...
function zs_ausgaben_install() {
  try {
	  ...
	  var css =  modulepath + '/lib/slick/slick.css';
	  drupalgap_add_css(css);
	  
	  var js =  modulepath + '/lib/slick/slick.min.js';
	  drupalgap_add_js(js);	  
...
* The callback for the "Ausgabe" Single page.
*/
function zs_ausgaben_ausgabe_leseprobe_page(artikelnr) 
{
	try {
	    if (artikelnr) 
	    {	
//	    	var frame = "<iframe src='../sites/bergsteiger.de/files/blick-ins-heft/501508/index.html' width='100%' scrolling='no' frameBorder='0' class='pdframe' onLoad='fixFrame();'></iframe>";
//	    	var frame = "<iframe src='http://bergsteiger.de/sites/bergsteiger.de/files/blick-ins-heft/501509/index.html' width='100%' scrolling='no' frameBorder='0' class='pdframe' onLoad='fixFrame();'></iframe>";
//	    	var frame = "<iframe src='http://selbermachen.de/ausgabe/vollbild/09-15-perfekte-pergola:-so-einfach-schaffen-sie-sich-eine-%C3%BCberdachte-oase' width='100%' scrolling='no' frameBorder='0' class='pdframe' onLoad='fixFrame();'></iframe>";
//	    	!!! WORKS !!! var frame = "<iframe src='http://zs.vh-digital.de/sites/selbermachen.de/files/blick-ins-heft/911509/index.html#undefined' width='100%' scrolling='no' frameBorder='0' class='pdframe' onLoad='fixFrame();'></iframe>";
	    	var frame = "<iframe src='http://bergsteiger.de/sites/bergsteiger.de/files/blick-ins-heft/" + artikelnr + "/index.html' width='100%' scrolling='no' frameBorder='0' class='pdframe' onLoad='fixFrame();'></iframe>";
	    	
	    	var fixJS = "<script>function fixFrame () {   " +
    		"iFrameContentHeight = $('.ausgabe___leseprobe.ui-page').height(); " +
    		"iFrameContentHeight = iFrameContentHeight - 44;" +
	    	"$('.pdframe').css('height', iFrameContentHeight)" +
	    	" }</script>";
	    ...
	    else { drupalgap_error(t('No artikelnr id provided!')); }
	}
...
* The callback for the "Ausgabe" Single page.
*/
function zs_ausgaben_ausgabe_voll_page(artikelnr) 
{
	try {
	    if (artikelnr) 
	    {	
	    	var frame = "<iframe src='app/modules/custom/zs_ausgaben/pdfjs/web/viewer.html' width='100%' scrolling='no' frameBorder='0' class='pdframe' onLoad='fixFrame();'></iframe>";
	    	var fixJS = "<script>function fixFrame () {   " +
    		"iFrameContentHeight = $('.ausgabe___leseprobe.ui-page').height(); " +
    		"iFrameContentHeight = iFrameContentHeight - 44;" +
	    	"$('.pdframe').css('height', iFrameContentHeight)" +
